...
def ZeroCoord(df_coord,l_nomi_pti,s_dates_zero):
    ...
    for nome_pto in l_nomi_pti:
        ...
        # check is coordinates value are a list or a scalar
        if isinstance(coord_i['E'], list):            
            E=coord_i['E'].mean()
            N=coord_i['N'].mean()
            H=coord_i['H'].mean()
            l_E.append(E)
            ...
            l_E.append(coord_i['E'])
            l_N.append(coord_i['N'])
            l_H.append(coord_i['H'])
...
# creation of labels  
def DatesLabelsCreation(df_coord,layer_name,i,text_h,drawing,modelspace):
...
def ScaleBarCreation(fs,df_coord,text_h,drawing,modelspace):
...
def DrawDisp(drawing,l_dates,df_coord,df_coord_zero,df_coord_rel,text_h,l_nomi_pti,modelspace):
    ...
    for i in range(1,len(l_dates)):
        ...
        DatesLabelsCreation(df_coord,layer_name,i,text_h,drawing,modelspace)
